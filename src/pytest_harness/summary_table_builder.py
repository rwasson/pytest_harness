...
def _build_summary_table(  # noqa: PLR0915
    *,
    ...
    # --- Reporting problems with test files ---
    if summary_data.not_processed_test_files:
        lines.extend(
            [
                "",
                (
                    f"Test files not processed, often due to import error "
        ...
        for test_file_name in summary_data.not_processed_test_files:
    ...
    if summary_data.no_tests_collected_test_files:
        lines.extend(
            [
                "",
                (
                    f"Test files with no collected tests ({summary_data.no_tests_collected_test_file_count}):"
                ),
                # "-" * divider_width,
            ]
        )

        for test_file_name in summary_data.no_tests_collected_test_files:
            lines.append(f"    {test_file_name}")
    ...
    # --- Reporting problems with test functions ---

    problem_label = "Flagged test functions by test file"

    problem_file_count = len(summary_data.problem_test_files)
    ...
    if summary_data.problem_test_files:
        ...
        for problem_record in summary_data.problem_test_files:
            lines.append(f"    {problem_record.test_file_name}")

            if problem_record.has_failures:
                lines.append(f"        Failed ({problem_record.failed_test_count}):")

                for test_name in problem_record.failed_test_function_names:
                    lines.append(f"            {test_name}")

            if problem_record.has_errors:
                lines.append(f"        Error ({problem_record.error_test_count}):")

                for test_name in problem_record.error_test_function_names:
                    lines.append(f"            {test_name}")

            if problem_record.has_xpasses:
                lines.append(f"        XPassed ({problem_record.xpassed_test_count}):")

                for test_name in problem_record.xpassed_test_function_names:
                    lines.append(f"            {test_name}")

            if show_skipped_and_xfailed and problem_record.has_skips:
                lines.append(f"        Skipped ({problem_record.skipped_test_count}):")

                for test_name in problem_record.skipped_test_function_names:
                    lines.append(f"            {test_name}")

            if show_skipped_and_xfailed and problem_record.has_xfails:
                lines.append(f"        XFailed ({problem_record.xfailed_test_count}):")

                for test_name in problem_record.xfailed_test_function_names:
                    lines.append(f"            {test_name}")
    ...
    if show_source_file_coverage:
        ...
        lines.extend(
            [
                # "",
                # "Statement coverage by source file:",
                # "-" * divider_width,
                "",
                "",
                (
